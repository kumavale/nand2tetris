...
pub fn write_push(segment: &str, index: u32, file: &str) -> String {
    let comment = format!("\n// push {} {}", segment, index);

    if segment == "static" {
        let asm_segment = format!("@{}.{} D=M @SP A=M M=D {}", file, index, sp_up());
    ...
    let segment_asm = match segment {
        ...
        "pointer" => match index {
            0 => "@THIS D=M",
            1 => "@THAT D=M",
            _ => panic!("A pointer value of '{}' cannot be pushed from THIS or THAT.", index),
        },
        _ => {
    ...
    let asm_string = format!("@{} {} @SP A=M M=D {}", index, segment_asm, sp_up());
    comment + &asm_new_line_concat(&asm_string)
}

// Translates parsed pop-commands (Com::Pop) into HACK-ASM
pub fn write_pop(segment: &str, index: u32, dest_id: usize, file: &str) -> String {
    let comment = format!("\n// pop {} {}", segment, index);
    ...
    if segment == "pointer" {
        ...
        let segment_asm = format!("{} @SP A=M D=M @{}.{} M=D", sp_down(), file, index);
        return comment + &asm_new_line_concat(&segment_asm);
    }
